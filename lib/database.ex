...
defmodule Database do
  def start(config, server_id) do
    db = %{config: config, id: server_id, seqnum: 0, balances: Map.new()}
    Database.next(db)
  end

  # setters
  def seqnum(db, v), do: Map.put(db, :seqnum, v)
  def balances(db, i, v), do: Map.put(db, :balances, Map.put(db.balances, i, v))

  def next(db) do
    receive do
      # should send a result back, but assuming always okay
      {:EXECUTE, command} ->
        ...
        Monitor.notify(db, {:DB_UPDATE, db.id, db.seqnum, command})
