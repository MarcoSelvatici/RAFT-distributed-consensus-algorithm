...
defmodule Client do
  ...
  def next(c) do
    ...
    after
      c.config.client_sleep ->
        ...
        # unique id for cmd
        uid = {c.id, c.cmd_seqnum}

        client_request = {:CLIENT_REQUEST, %{clientP: self(), uid: uid, cmd: cmd}}

        # result not used
        {c, _client_result} = Client.send_request(c, client_request)
  ...
  def send_request(c, client_request) do
    ...
    receive do
      {:CLIENT_REPLY, result} ->
        # update leaderP
        c = Map.put(c, :leaderP, result.leaderP)
      ...
      {:CLIENT_STOP} ->
        Client.stop(c)
    after
      c.config.client_timeout ->
        # leader probably crashed, retry command with different server
        non_leader =
          for server <- c.servers, server != c.leaderP do
            server
          end
