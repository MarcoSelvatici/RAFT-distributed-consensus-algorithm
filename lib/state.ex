# Marco Selvatici, ms8817

# distributed algorithms, n.dulay, 4 feb 2020
# coursework, raft consensus, v1

defmodule State do
  # *** short-names: s for 'server', m for 'message'

  def initialise(config, server_id, servers, databaseP) do
    %{
      # system configuration parameters (from DAC)
      config: config,
      # server's process id
      selfP: self(),
      # server's id (simple int)
      id: server_id,
      # list of process id's of servers
      servers: servers,
      # process id of local database
      databaseP: databaseP,
      majority: div(length(servers), 2),
      # count of votes incl. self
      votes: 0,

      # -- various process id's - omitted

      # -- raft persistent data
      # -- update on stable storage before replying to requests
      curr_term: 0,
      voted_for: nil,
      log: [],

      # -- raft non-persistent data
      role: :FOLLOWER,
      commit_index: 0,
      last_applied: 0,
      next_index: Map.new(),
      match_index: Map.new()

      # add additional state variables of interest
    }
  end

  # setters
  def votes(s, v), do: Map.put(s, :votes, v)

  # setters for raft state
  def role(s, v), do: Map.put(s, :role, v)
  def curr_term(s, v), do: Map.put(s, :curr_term, v)
  def voted_for(s, v), do: Map.put(s, :voted_for, v)
  def log(s, v), do: Map.put(s, :log, v)
  def commit_index(s, v), do: Map.put(s, :commit_index, v)
  def last_applied(s, v), do: Map.put(s, :last_applied, v)
  # sets new next_index map
  def next_index(s, v), do: Map.put(s, :next_index, v)
  def next_index(s, i, v), do: Map.put(s, :next_index, Map.put(s.next_index, i, v))
  # sets new  match_index map
  def match_index(s, v), do: Map.put(s, :match_index, v)
  def match_index(s, i, v), do: Map.put(s, :match_index, Map.put(s.match_index, i, v))

  # add additional setters
end

# State
